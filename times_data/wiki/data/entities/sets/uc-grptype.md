---
type: entity
title: "uc_grptype"
aliases: []
tags: [set, internal]
sources:
  - raw/reference/sets.json
  - raw/docs/part-2/02-sets.md
related: []
last_updated: 2026-04-11
---

# uc_grptype

**Category:** Internal
**Aliases:** None

## Description

Index of internally fixed key types of variables: ACT, FLO, IRE, CAP, NCAP, COMPRD, COMNET, COMCON, UCN These are used in association with the user constraints.

## Usage Notes

See the [Sets chapter](../../raw/docs/part-2/02-sets.md) in the TIMES Reference Manual for full details on how this set is used in the model equations.
